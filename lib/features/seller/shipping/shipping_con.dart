...
class ShippingCon extends GetxController {
  ...
  /// Per shipment row action: `"$id|dispatch"`, `"$id|track"`, `"$id|update"`, `"$id|delayed"`.
  final RxnString shipmentRowActionKey = RxnString();

  bool isShipmentRowActionLoading(String shipmentId, String slug) =>
      shipmentRowActionKey.value == '$shipmentId|$slug';

  Future<void> _runShipmentRowAction(
      String shipmentId, String slug, void Function() op) async {
    if (shipmentRowActionKey.value != null) return;
    shipmentRowActionKey.value = '$shipmentId|$slug';
    await Future<void>.delayed(const Duration(milliseconds: 200));
    op();
    shipmentRowActionKey.value = null;
  }

  Future<void> dispatchShipmentRow(String id) async {
    await _runShipmentRowAction(id, 'dispatch', () => markAsDispatched(id));
  }

  Future<void> trackShipmentRow(String id) async {
    await _runShipmentRowAction(id, 'track', () => trackShipmentById(id));
  }

  Future<void> updateShipmentRowStub(String id) async {
    await _runShipmentRowAction(id, 'update', () {});
  }

  Future<void> delayedSidebarShipmentAction(ShipmentModel s) async {
    await _runShipmentRowAction(s.id, 'delayed', () {
      if (s.status == ShipmentStatus.pending) {
        markAsDispatched(s.id);
      } else {
        trackShipmentById(s.id);
      }
    });
  }
  ...
  final RxList<ShipmentModel> shipments = <ShipmentModel>[
    ShipmentModel(
      ...
      avatarHex: "0xFF7C3AED", // String hex pass kiya hai
  ...
  final RxList<DeliveryPartnerModel> partners = <DeliveryPartnerModel>[
    DeliveryPartnerModel(
      ...
      name: 'TCS Express',
      emoji: '🚚',
      logoBgHex: "0xFFFFF3CD", // Passed string hex instead of color object
  ...
  void trackShipment(ShipmentModel shipment) => trackShipmentById(shipment.id);

  void trackShipmentById(String id) {
    // navigate to tracking detail (placeholder)
  }
