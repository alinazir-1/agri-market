...
class ReviewsCon extends GetxController {
  ...
  final RxBool isReplySubmitting = false.obs;
  ...
  final RxList<ReviewModel> reviews = <ReviewModel>[
    ReviewModel(
      ...
      avatarHex: '0xFF7C3AED', // Converted color to String hex as per model
      productId: 'P1',
  ...
  bool submitReply(String reviewId) {
    ...
    reviews.refresh(); // Update UI
    replyController.clear();
    return true;
  }

  Future<void> submitReplyAndCloseDialog(String reviewId) async {
    if (isReplySubmitting.value) return;
    if (replyController.text.trim().isEmpty) return;
    isReplySubmitting.value = true;
    await Future<void>.delayed(const Duration(milliseconds: 160));
    try {
      final ok = submitReply(reviewId);
      if (ok) Get.back<void>();
    } finally {
